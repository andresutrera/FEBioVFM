...
inline bool map_node(const MeshDims& dims, int nodeId, std::size_t& idx) {
  ...
  if (it == dims.nodeId2idx.end()) return false;
...
bool VFMLoader::load_params(const XMLInput& dto, VFMState& state, std::string& err)
{
  ...
  for (const auto& p : dto.parameters) { // XMLInput::Param {name, init, lo, hi, scale}
    if (p.name.empty()) { err = "Parameters: empty name"; return false; }
    if (!std::isfinite(p.init)) { err = "Parameters[" + p.name + "]: non-finite init"; return false; }
    if (!valid_bounds(p.lo, p.hi)) { err = "Parameters[" + p.name + "]: invalid bounds"; return false; }
    if (!in_bounds(p.init, p.lo, p.hi)) { err = "Parameters[" + p.name + "]: init out of bounds"; return false; }
    if (!std::isfinite(p.scale) || p.scale == 0.0) { err = "Parameters[" + p.name + "]: invalid scale"; return false; }
...
bool VFMLoader::load_measuredU(const XMLInput& dto, const MeshDims& dims, MeasuredData& out, std::string& err)
{
  ...
  for (const auto& ts : dto.measuredU) {
    ...
    for (const auto& s : ts.nodes) {
      std::size_t i;
      if (!map_node(dims, s.id, i)) { err = "Unknown node id in measuredU: " + std::to_string(s.id); return false; }
...
bool VFMLoader::load_virtualU(const XMLInput& dto, const MeshDims& dims, VirtualFields& out, std::string& err)
{
  ...
  for (std::size_t v=0; v<dto.virtualU.size(); ++v) {
    ...
    for (const auto& ts : vf.times) {
      ...
      for (const auto& s : ts.nodes) {
        std::size_t i;
        if (!map_node(dims, s.id, i)) { err = "Unknown node id in virtualU: " + std::to_string(s.id); return false; }
...
{
  // Node-resolved loads need a surface→node map. If you do not have it yet,
  // just size times and leave zeros, or skip this until you implement surf-maps.
  out = MeasuredLoad{};
  out.setNodalSize(dims.nNodes);

  for (const auto& tl : dto.measuredLoads) {
    (void)out.addTime(); // placeholder; fill when surface node mapping exists
  }
  return true;
}
