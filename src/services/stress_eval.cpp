...
bool StressEval::cauchy(const Deformations &F, Stresses &out, const IMaterialProvider &mat, std::string &err)
{
  ...
  { // assume frames already created to match
    for (size_t k = out.nTimes(); k < F.nTimes(); ++k)
      (void)out.addTime();
  }
  for (TimeIdx t = 0; t < (TimeIdx)F.nTimes(); ++t)
  {
    ...
    for (size_t e = 0; e < ne; ++e)
    {
      ...
      for (size_t g = 0; g < ng; ++g)
      {
        ...
        mat3ds s;
        if (!mat.evalCauchy(e, g, Fin, s, err))
          return false;
        out.setSigma(t, e, g, mat3d(s));
      }
    }
  }
  return true;
...
bool StressEval::first_piola(const Deformations &F, const Stresses &S, Stresses &outP, std::string &err)
{
  ...
  {
    for (size_t k = outP.nTimes(); k < F.nTimes(); ++k)
      (void)outP.addTime();
  }
  for (TimeIdx t = 0; t < (TimeIdx)F.nTimes(); ++t)
  {
