...
bool VFMTask::Init(const char* xmlPath)
{
    ...
    MeshDims dims; MeshConn conn; MeshQuad quad;
    if (!build_mesh_info(*GetFEModel(), dims, conn, quad, err)) { feLogError(err.c_str()); return false; }
    ...
    if (!VFMLoader::load_measuredU(m_input, dims, measured, err)) { feLogError(err.c_str()); return false; }
    if (!VFMLoader::load_virtualU (m_input, dims, virtuals, err)) { feLogError(err.c_str()); return false; }
    if (!VFMLoader::load_measuredF(m_input, dims, loads, err))    { feLogError(err.c_str()); return false; }
    if (!VFMLoader::load_params(m_input, m_state, err)) { feLogError(err.c_str()); return false; }
    ...
    // after compute_*
    if (!err.empty()) { feLogError(err.c_str()); return false; }
    ...
    if (!StressEval::cauchy(m_state.def, m_state.stresses, mat, err)) { feLogError(err.c_str()); return false; }

    Stresses P; P.setElemShape(quad.gpPerElem);
    for (size_t k=0;k<m_state.def.nTimes();++k) (void)P.addTime();
    if (!StressEval::first_piola(m_state.def, m_state.stresses, P, err)) { feLogError(err.c_str()); return false; }
